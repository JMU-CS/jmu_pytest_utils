...
OUTPUT_FILES = ["NAME.txt", "OUTPUT.txt", "results.json", ".coverage"]
...
def clean_cmd():
    ...
    if os.path.exists("config.sh"):
        ...
        make_cfg()
        with open("config.bak") as f1, open("config.sh") as f2:
            if f1.read() == f2.read():
    ...
    for filename in CONFIG_FILES + SCRIPT_FILES:
        if os.path.exists(filename):
            ...
            with open(filename) as f1, open(template) as f2:
                if f1.read() == f2.read():
